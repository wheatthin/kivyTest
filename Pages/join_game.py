...
class JoinGame(Screen):
    ...
    async def send_join_game(self, username, match_id):
        async with websockets.connect("ws://localhost:8765") as websocket:
            message = json.dumps({"action": "join_game", "username": username, "match_id": match_id})
    ...
    def join_game(self, instance):
        ...
        if username and match_id:
            Clock.schedule_once(lambda dt: asyncio.run(self.send_join_game(username, match_id)))
            self.manager.get_screen('lobby').add_player(username, match_id)
            self.manager.current = 'lobby'

    def update_font_size(self, font_size):
        self.title_label.font_size = font_size
        self.username_input.font_size = font_size
        self.username_input.height = 40  # Fixed height
        self.match_id_input.font_size = font_size
        self.match_id_input.height = 40  # Fixed height
